...
fn main() {
    ...
    for entry in WalkDir::new("//192.168.3.10/jav/media/").into_iter().filter_map(|e| e.ok()) {
    // for entry in WalkDir::new("//192.168.3.10/jav/media/hd/#三宮つばき/") {
        let path = entry.path();
        cnt += 1;
        ...
        stdout
            .write_all(format!("scan files {:?}", filename).as_bytes())
            .unwrap();
